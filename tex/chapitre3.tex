...
La nécessité d'avoir une évaluation robuste de ces similarités entre voxels a nécessitée la prise en compte leur proche voisinage, conduisant à la définition d'une notion de patches.
...
Certaines méthodologies de types FCM ont inclu une prise en compte de la redondance de l'information en favorisant les voxels dont l'intensité et les coordonnées spatiales sont les plus proches de celles du voxel courant (voir en Section~\ref{subsubsec:fcm:ponderation}).
...
Les moyennes non-locales ont également été étudiées directement depuis l'espace des patch.
...
La figure~\ref{fig:nlm:kernels} illustrent cette problématique en comparant différents noyaux et démontre l'utilité d'un noyau compact polynomiale par rapport au noyau gaussien (en ne prenant en compte que les patches \og proches \fg{} du patch central) et par rapport au noyau plat (qui n'est pas une fonction continue et ne pondère pas les patches).
...
La première est celle de \cite{VanDeVille:TIP:2011} qui exécute les moyennes non-locales avec plusieurs tailles de patches et de zones de recherche, et effectue une combinaison de ces résultats afin d'obtenir l'estimation la plus robuste possible en chaque élément de l'image.
...
Les travaux de \cite{Gilboa:MMS:2008} se sont focalisés sur la définition d'un opérateur de gradient et d'un opérateur de divergence non-locaux de manière à étendre les techniques fondées sur des équations aux dérivées partielles (EDP) ou des approches variationnelles.
...
L'article de \cite{Rousseau:TMI:2011} décrit une méthodologie cherchant les patches similaires dans une base d'images, après une recalage affine et une égalisation d'histogramme.
...
Les moyennes non-locales ont également été introduites dans les lignes de niveaux par~\cite{Jung:SSVM:2011} et ont permis la segmentation de structures dont l'intensité varie de façon lisse, ce qui peut se révéler intéréssant dans le cas ou un biais est présent.
...
Les paramètres $n_j$ et $m_j$ sont donc fixé à : $(n_j, m_j) = (8, 8)$ (voir la figure~\ref{FIG:PARAM:BRAINWEB:BIAS}) car le résultat de ce couple de valeur est proche du résultat optimal et permet la diminution du temps de calcul.
...
Un exemple est donné par le cas n\textdegree11 où il apparaît clairement que le matière grise est sur-segmentée, constituant ainsi un sur-ensemble de la matière grise réelle, expliquant les plus faibles scores obtenus pour ce cas que pour les autres.
...
Une autre solution serait de décomposer l'image en sous-volumes disjoints, d'effectuer une évaluation locale du modèle dans ces sous-volumes puis d'effectuer une interpolation des huit modèles les plus proches en chaque voxels pour obtenir une estimation des centroïdes au voxel courant.
...
Cependant, l'utilisation conjointe de ces termes non-locaux ne semble pas apporté de gain significatif dans un environnement à la fois biaisé et bruité.
