...
Les deux inovations permettant permettant une grande efficacité de ce filtre sont la non-localité et l'utilisation de patches.
...
\end{enumerate}
Cependant, il est montré que ce filtre reste moins efficace que d'autres méthodes \cite{Goossens:LNLA:2008} développées récemment comme la méthode BM3D (\emph{Bloc-Matching and 3D filtering}) \cite{Dabov:TIP:2007}.
Cet algorithme est également fondé sur un traitement par patch, mais traite l'image en groupant les patches similaires dans des piles, ces dernières étant filtrés par des méthodes d'ondelettes.
Les estimateurs ainsi estimés sont alors reprojetés dans le domaine des voxels et combinés de manière à obtenir une estimation de la valeur en chaque voxel.



Plusieurs paramètres influent sur le comportement des moyennes non-locales. 
Ces paramètres peuvent être numériques, tel que le paramètre de lissage $h$ ou la taille des patches $\lvert P_{j} \rvert$.
...
FIXME : Ajouter \cite{Deledalle:JMIV:2011} pour les formes de patches adaptatives.
...
\`A un niveau plus théorique, l'article de \cite{Katkovnik:IJCV:2010} assimile la première version des moyennes non-locales à une régression linéaire de niveau $0$ et l'étend à des niveaux supérieurs.
...
Il est étendu à la prise en compte des patches par l'article de \cite{Katkovnik:IJCV:2010}, définissant ainsi le \emph{Generalized NDS (GNDS)}, dont le NDS est un cas particulier.
