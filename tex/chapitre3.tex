...
Elle prend en compte l'approche des moyennes non-locales (en anglais, \emph{non-local means}) proposée en 2005.
...
La nécessité d'avoir une évaluation robuste de ces similarités entre voxels a conduit à prendre en compte leur proche voisinage, conduisant à la définition de patchs.

L'utilisation de la redondance de l'information de l'image est une option peu explorée en segmentation cérébrale.
Comme nous l'avons vu précédemment, de nombreuses méthodes utilisent une modélisation spécifique d'un champ de biais pour la correction de l'inhomogénéité de l'intensité ou une approche prenant en compte la classification des voxels environnants pour corriger le bruit.
Certaines méthodologies de types FCM ont inclus une prise en compte de la redondance de l'information en favorisant les voxels dont l'intensité et les coordonées spatiales sont les plus proche de celles du voxel courant.
Cependant, les travaux sur les méthodes de débruitage par patchs (dont l'approche non-locale) ont montré que ces dernières étaient plus efficaces car elles offrent une estimation de la similarité plus fiable qu'une comparaison voxel à voxel.
...
Concernant la prise en compte du biais, certaines méthodologies de type FCM ont introduit une évaluation locale des centroïdes afin de ne pas avoir à évaluer le champ de biais directement.
Cependant, la taille du voisinage utilisé pour estimer ces centroïdes locaux est difficile à fixer et une mauvaise estimation à un voxel donné ne peut pas être corrigée.
...
Le calcul de cette dernière est effectué par la comparaison entre les patchs respectifs des voxels considérés et est contrôlé par le choix de la norme $\lVert \cdotp \rVert$.
...
\end{equation}
avec $\sum_{j'=1}^{N} \omega_{nl}(j,j') = 1$ et est donc directement lié au calcul de la similarité entre les voxels de la zone de recherche.
...
De nombreux articles ont discuté les limites des moyennes non-locales et l'influence des paramètres.
...
La taille de cette zone de recherche peut également être ajustée localement en fonction du niveau de bruit local, comme dans l'article de \cite{Kervrann:TIP:2006}.

Des stratégies ont été adoptées pour calculer de manière automatique le paramètre $h$ \cite{Tasdizen:ICIP:2008,Coupe:TMI:2008}. 
...
L'article de Buades \cite{Buades:MMS:2005} utilise un noyau de type gaussien $K(x) = e^{-x}$, qui est encore largement utilisé dans la littérature.
...
En effet, le noyau gaussien a l'inconvénient de donner une valeur proche de zéro, mais non nulle, aux voxels peu similaires au voxel central, ce qui tend à perturber l'efficacité du débruitage.
...
Cependant, l'utilisation de noyaux compacts apporte une réponse plus naturelle à ce problème en définissant l'influence des voxels peu similaires au voxel central comme nulle (voir la figure~\ref{fig:nlm:kernels}).
...
De manière générale, l'utilisation la norme euclidienne pour le calcul de la similarité entre deux patchs a peu été remis en cause \cite{Salmon:2010}.
...
L'article de \cite{Rousseau:TMI:2011} décrit une méthodologie cherchant les patchs similaires dans une base d'image, après une recalage affine et une égalisation d'histogramme.
...
Les limites et les différentes approches concernant les moyennes non-locales sont explorés plus en détail dans le travail de Salmon \cite{Salmon:2010}.


\subsection{Segmentation par FCM non-local}
...
L'idée sous-jacente est qu'il existe des parties de l'image présentant peu de similaritée entre les voxels (par exemple, dans une IRM cérébrale, autour des sillons cérébraux) où il est plus efficace de prendre en compte une similarité entre les voxels directement plutôt qu'entre les patchs.
...
Nous présentons dans cette section une amélioration de l'algorithme FCM prenant en compte les moyennes non-locales au sein du terme d'attache aux données de manière à améliorer la prise en compte du biais en intensité et au sein du terme de régularisation afin d'améliorer la correction du bruit.
...
Une même intensité peut donc être étiquettée différemment selon l'estimation locale du modèle.
La taille des sous-volumes est cependant cruciale car un sous-volume trop important sera sensible à l'inhomogénéité tandis qu'un volume trop réduit risque de conduire à une mauvaise évaluation du modèle d'intensité local. 
...
Les expériences permettant la validation des FCM non-locaux sont menées tout d'abord sur des images simulées fournies par la base BrainWeb \cite{Kwann:VBC:1996, Cocosco:NeuroImage:1997}, puis sur des cas réels fournis par la base IBSR (\emph{Internet Brain Segmentation Repository}).
...
En effet, les voxels de faible similarité avec le voxel courant sont pris en compte dans le calcul des pondérations, ce qui peut entraîner un biais dans le calcul de l'attache aux données locale.
...
Cependant, les méthodes fondées sur les champs et chaînes de Markov semblent moins efficaces concernant la segmentation de la matière grise.
...
Cependant, le deuxième obtient des résultats plus stables, illustré par une dispersion des scores équivalente aussi concernant la segmentation de la matière grise que celui de la matière blanche.
...
Différentes voies doivent être explorées afin de diminuer le temps d'exécution du logiciel avant qu'une utilisation clinique soit envisageable.
