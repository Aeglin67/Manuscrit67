...
L'idée générale est de profiter de la redondance de l'information au sein d'une image pour estimer une valeur corrigée en chaque voxel.
La nécessité d'avoir une évaluation robuste de ces similarités entre voxels a conduit à l'introduction de patchs (voisinage autour d'un voxel).
...
Certaines méthodologies de types FCM ont inclus une forme de prise en compte de la redondance en favorisant les voxels dont l'intensité et la position spatiale sont les plus proche de celles du voxel courant.
...
L'intégration de l'approche non-locale au sein du terme de régularisation va donc permettre une meilleure prise en compte de l'environnement autour du voxel courant pour effectuer la régularisation.
...
Le débruitage par moyennes non-locales et l'évaluation des redondances reposent sur des patchs, qui sont définis comme un petit voisinage cubique autour d'un voxel d'intérêt $j$, de rayon $W$ et indexé par son voxel central (voir la figure~\ref{fig:mean_filter}).
...
Le calcul de la similarité est effectué par la comparaison entre les patchs respectifs des voxels considérés et est contrôlé par le choix de la norme $\lVert \cdotp \rVert$.
...
\end{equation}
avec $\sum_{j'=1}^{N} \omega_{nl}(j,j') = 1$ et est donc directement liée au calcul de la similarité entre les voxels de la zone de recherche.

\subsection{Etat de l'art}

Plusieurs paramètres influent sur le comportement des moyennes non-locales. 
Ces paramètres peuvent être numériques, tel que le paramètre de lissage $h$ ou la taille des patchs $\lvert P_{j} \rvert$.
% ou la taille de la zone de recherche $\lvert \Omega^{R}_{j} \rvert$.
D'autres sont plus difficilement quantifiables, tels que le choix de la fonction noyau $K(\cdot)$ et de la norme $\lVert \cdot \rVert$ utilisée pour le calcul de la similarité.
...
Pour des raisons pratiques, notamment le temps d'exécution, le calcul des poids non-locaux a été restreint à une zone de recherche $\Omega_{j}^{R}$ autour du voxel $j$.
L'article fondateur de \cite{Buades:MMS:2005} propose déjà cette solution.
Cependant, la taille de cette zone est un paramètre qui a peu été étudié et qui est assez difficile à évaluer.
Les différentes études concluent que cette taille est à ajuster en fonction de l'image considérée \cite{Salmon:2010}.
La taille de cette zone de recherche peut également être ajusté localement en fonction du niveau de bruit local, comme dans l'article de \cite{Kervrann:TIP:2006}.

Des stratégies ont très vite été adoptées pour calculer de manière automatique le paramètre $h$ \cite{Tasdizen:ICIP:2008,Coupe:TMI:2008}. 
L'article fondateur de \cite{Buades:MMS:2005} propose de fixer ce paramètre selon la règle : $h = 10\sigma$, où $\sigma$ est l'écart-type du bruit.
...
Un dernier exemple d'ajustement est le travail de \cite{Tasdizen:TIP:2009} qui réalise un sorte d'apprentissage sur différentes images ayant un niveau de bruit connu.
La relation linéaire entre $h$ et $\sigma$ est mise en évidence et ses paramètres calculés en fonction du niveau de bruit et du paramètre $h$ permettant un débruitage optimale.

\begin{figure}[!t]
\begin{center}
\includegraphics[height=55mm]{eps/chapitre3/kernels.eps}
\end{center}
\caption[Différents noyaux utilisés dans le cadre des moyennes non-locales]{\emph{Différents noyaux utilisés dans le cadre des moyennes non-locales. Noir : noyau plat. Vert : noyau gaussien $K(x) = \exp{(-x)}$. Rouge : noyau compact polynomial de degré 4. Bleu : noyau compact polynomial de degré 6.}}
...
En effet, le noyaux gaussien a l'inconvénient de donner une valeur proche de zéro, mais non nulle, aux voxels peu similaires au voxel central, ce qui tend à perturber l'efficacité du débruitage.
...
Cependant, l'utilisation de noyaux compacts apporte une réponse plus naturel à ce problème en définissant l'influence des voxels peu similaires au voxel central comme nulle (voir la figure~\ref{fig:nlm:kernels}).
De plus, ces noyaux ont l'avantage d'être continus.
Un noyau plat peut être employé afin de privilégier la vitesse de calcul et il est courant de trouver des noyaux compacts polynomiaux de degré $4$ \cite{Goossens:LNLA:2008} ou de degré $6$ \cite{Duval:2010}.
...
Cependant, quelques alternatives au calcul direct de la distance entre les patchs peuvent être notés.
L'article fondateur de \cite{Buades:MMS:2005} inclue également une pondération en fonction de la distance des voxels par rapport au voxel central.
Cependant, il semble que cette pondération ait été peu retenue dans les travaux ultérieurs.
Les travaux de \cite{Azzabou:ICIP:2007} et \cite{Tasdizen:TIP:2009} utilisent un dictionnaire issue de l'image et calculé par une analyse des composantes principales (ACP) de la matrice de covariance construite à partir de l'ensemble des voisinages de l'image.
La comparaison des patchs se fait alors dans l'espace construit à partir des composantes présentant les valeurs propres les plus élevées. 
...
De plus, le bruit peut ne pas être uniforme sur l'image et la taille du patch doit donc être idéalement défini localement.
...
Notons l'article de \cite{Manjon:JMRI:2010} introduisant des patchs adaptatifs de manière à débruiter une IRM présentant un bruit variable.
L'article de \cite{Kervrann:IJCV:2008} peut également être mentionné pour l'utilisation de cette technique.
% Une technique de patchs adaptatifs a été implémentée par \cite{Kervrann:TIP:2006} dans l'
...
Ces méthodes de régularisation peuvent être vues comme des versions discrète d'un formalisme plus général appellé \emph{Non local data and smoothness term (NDS)} \cite{Mrazek:2006}. 
Ici, le terme \emph{non-local} est utilisé car des interactions avec des voxels plus éloignés que le voisinage immédiat sont prises en compte.
...
Il est étendu à la prise en compte des patchs par l'article de \cite{Katkovnik:IJCV:2010}, définissant ainsi le \emph{Generalized NDS (GNDS)}, dont le NDS est un cas particulier.
...
Nous pouvons citer les travaux de \cite{Bougleux:IJCV:2009} déjà évoqués, ainsi que ceux de \cite{Mignotte:PRL:2008} en déconvolution et ceux de \cite{Rousseau:MIA:2010} en reconstruction de volumes 3D.
En déconvolution, les moyennes non-locales sont utilisées de manière à favoriser les images présentant un haut niveau de redondance, soit dans l'idéal des images invariantes au filtre non-locale.

Enfin, la segmentation d'images est un domaines où les moyennes non-locales ont également trouvés une application.
Les travaux de \cite{Gilboa:MMS:2008} se sont focalisés sur la définition d'opérateurs gradient et divergence non-locaux de manière à étendre les techniques fondées sur des équations différentielles partielles (EDP) ou des approches variationnelles.
Les travaux de \cite{Bresson:2008} ont définis plusieurs fonctionnelles non-locales permettant une segmentation non-supervisée par contours actifs avec une approche variationnelle.
...
L'article de \cite{Rousseau:TMI:2011} décrit une méthodologie cherchant les patchs similaires dans les images de la base, après une recalage affine et une égalisation d'histogramme.
Cette méthododologie a l'avantage de ne pas nécessiter de recalage non-linéaire, très gourmand en temps de calcul, et base l'étape de fusion des labels sur les poids non-locaux calculés à travers la base.
...
Pour en savoir plus sur les limites et les différentes approches concernant les moyennes non-locales, le lecteur est invité à lire la thèse de Salmon \cite{Salmon:2010}.

FIXME : ajouter \cite{Jung:SSVM:2011}, level-set non locaux, avantage de segmenter des structures dont l'intensité varie de façon lisse (intéressant dans le cas d'un biais).

% Citations à rajouter : 
% \begin{itemize}
%         \item \cite{Bougleux:IJCV:2009} et \cite{Elmoataz:TIP:2008} : régularisation sur des graphes. Version discrète et généralisation de différents filtres existants montrant comment ils sont apparentés les uns aux autres.
%         \item \cite{Gilboa:MMS:2008} : Opérateurs non-locaux destinés à étendre les techniques connues fondées sur des EDP ou une approche varationelle au non-locale. Principal focus, les opérateurs différentiels discrétisés tels que gradients et divergence.
%         \item \cite{Pizarro:IJCV:2010} : Extension d'un formalisme continue et général du débruitage au domaine non-local dont les précédentes méthodes de régularisation sur graphe sont des versions discrètes.
%         \item \cite{Bresson:2008} : Segmentation non supervisée par contours actifs, approches variationnelles non-locales.
%         \item \cite{Tasdizen:TIP:2009} et \cite{Tasdizen:ICIP:2008} : Débruitage par moyenne non-locale utilisant l'analyse en composante principale (issues de la décomposition de la matrice de covariance du vecteur de l'ensemble des voisinages). La fenêtre $h$ est choisie comme une combinaison linéaire du niveau de bruit $\sigma$. Les paramètres de cette combinaison linéaire sont choisis de manière empirique.
%         \item \cite{Manjon:JMRI:2010} : Filtre avec patch adaptatif pour IRM ayant un bruit variable.
%         \item \cite{Kervrann:TIP:2006} : Filtre à moyenne non-locale à patchs fixes mais à zone de recherche variable.
%         \item \cite{Azzabou:ICIP:2007} : Comparaison par rapport à un dictionaire construit à partir de l'ACP de l'ensemble des configuration de patchs trouvées dans l'image.
%         \item \cite{Duval:2010} : Choix local de $h$ car même si il existe une relation linéaire entre $\sigma$ et $h$, elle est différente d'une image à une autre. Privilégie les noyaux compactes.
% \end{itemize}
% 

\subsection{Segmentation par FCM non-locale}
...
\end{equation}
où $\omega_{nl}(j,j')$ représente le poid non-local entre les voxels $j$ et $j'$.
...
L'article de~\cite{Pham:CVIU:2001} a une approche légèrement différente étant donné que le lissage ne se fait qu'en prenant en compte l'ensemble des classes moins la classe courante, ce qui permet de favoriser le terme d'attache aux données si la classe courante est bien représentée dans le voisinage. 
...
L'hypothèse que nous faisons est que si les patchs de deux voxels sont similaires, alors ils appartiennent au même tissu.
...
Rappelons que $L_{k}=[\![1,C]\!]\setminus \{k\} = \{1, \ldots, k-1, k+1, \ldots, C\}$ et représente l'ensemble des classes exceptée la classe dont on effectue la régularisation et $\Omega^{R_{r}}_{j}$ représente la zone de recherche centrée autour du voxel $j$ destinée à calculer les poids non-locaux pour la régularisation.
...
\right.
\end{equation}
Il est important de noter que les poids $\omega_{nl}(\cdot,\cdot)$ présents dans le terme d'attache aux données et le terme de régularisation sont distincts car les zone de recherche $\Omega^{R_d}_{j}$ et $\Omega^{R_r}_{j}$ ne sont pas nécessairement identiques.
...
\begin{equation}
 u_{jk} = \frac{(\sum_{j' \in \Omega^{R_d}} \omega(j,j')\lVert \mathbf{y}_{j} - \mathbf{v}_{k} \rVert^{2} + \beta \sum_{j'' \in \Omega^{R_r}} \sum_{m \in M_{k}} u_{j''m}^{q})^{\frac{-1}{q-1}}}{\sum_{k=1}^{C}(\sum_{j' \in \Omega^{R_d}} \omega(j,j')\lVert \mathbf{y}_{j} - \mathbf{v}_{k} \rVert^{2} + \beta \sum_{j'' \in \Omega^{R_r}} \sum_{m \in M_{k}} u_{j''m}^{q})^{\frac{-1}{q-1}}}
\end{equation}
...
Les expériences permettant la validation des FCM non-locaux sont menées tout d'abord sur des images simulées fournies par la base BrainWeb \cite{Cocosco:NeuroImage:1997, Kwann:VBC:1996}, puis sur des cas réels fournis par la base IBSR (\emph{Internet Brain Segmentation Repository}).
...
La première partie de la courbe, de $m_j = 5$ à $m_j = 9$, montre comme attendu qu'à sous-volume égal, l'accroissement de la taille de zone de recherche permet d'obtenir une meilleure segmentation par la prise en compte des modèles voisins et la pondération en résultant.
Cependant, un fléchissement est observé à $m_j = 10$ pour $n_j = 8$ et $n_j = 9$.
Les différents travaux sur les moyennes non-locales pointaient la faible efficacité d'une zone de recherche trop grande, surtout dans le cas d'un noyaux à support infini tel que le noyau gaussien. 
En effet, les voxels de faibles similarité avec le voxel courant sont pris en compte dans le calcul des pondérations, ce qui peut entraîné un biais dans le calcul de l'attache aux données locale.
Ceci, couplé à un voisinage $M_j$ trop important rendu le calcul des centroïdes locaux sensibles à l'inhomogénéité en intensité, peut expliquer la baisse des performances de l'algorithme FCM non-local avec des paramètres $n_j$ et $m_j$ trop importants.
...
\end{tabular}
% \vspace{2mm}
\caption[Coefficients Dice issus de l'application de différentes segmentations à une image pondérée en T1 présentant un biais en intensité de 20~\%]{\emph{Application de différentes segmentations à une image pondérée en T1 présentant un biais en intensité de 20~\%. Comparaison des différents coefficient Dice pour la segmentation de la matière grise et de la matière blanche. Paramètres pour NL-FCM : $n_j = 8$ et $m_j = 8$. \label{TAB:DICE:BRAINWEB:BIAS}}}
\end{center}
\end{table}
...
Nous pouvons observer que l'algorithme FCM non local atteint des performances similaires à HMC.
...
La méthode HMC \cite{Bricq:MIA:2008} semble plus avantageuse car elle obtient la meilleure moyenne absolue, mais également la plus faible dispersion.
Cependant, les méthodes fondées sur les champs et chaînes de Markov semblent moins efficace concernant la segmentation de la matière grise.
...
Cependant, le deuxième obtient des résultats plus stable, illustré par une dispersion des scores équivalente aussi concernant la segmentation de la matière grise que celui de la matière blanche.
...
Au cours de ce chapitre, nous avons présenté une nouvelle méthode de segmentation fondée sur FCM, intégrant les moyennes non-locales.
...
Le deuxième permet un lissage plus pertinent de la segmentation et accroit les performances de FCM dans un environnement bruité.

Les résutats obtenus par le terme de régularisation non-locale démontrent que cette méthodologie est tout indiquée si un bruit pollue l'image à segmenter.
...
Le terme d'attache-aux-données repose sur la calcul de centroïdes locaux et sur le calcul des poids non-locaux permettant une pondération des modèles locaux environnants.
...
Différentes voies doivent être explorée afin de diminuer le temps d'exécution du logiciel avant qu'une utilisation clinique soit envisageable.
Le terme de régularisation ne nécessite aux contraire que le calcul des poids non-locaux dans une zone de recherche plus réduite ($5\times5\times5$).
Les temps de calculs sont alors réduit à une dizaine de minutes.

Les expériences menées afin d'évaluer les deux termes de la fonction d'énergie de cette nouvelle version de FCM séparément ont montré qu'ils obtenaient de meilleurs résultats que les autres méthodologies testée.
...
Une étude plus poussée des interractions entre ces deux termes est nécessaire afin de comprendre la façon dont ils fonctionnent ensemble et permettre cette amélioration.
