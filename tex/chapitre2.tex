...
Dans le même esprit d'utiliser plusieurs sources d'information pour guider le contour, l'article \cite{Ciofolo:MIA:2009} définit une méthodologie calculant la direction de propagation à partir d'un contrôleur flou prenant en compte un \emph{a priori} fourni par un atlas anatomique, l'intensité des voxels, la position du contour dans l'image et par rapport à d'autres objets.
...
L'algorithme des $C$-Moyennes floues (FCM), décrit en section \ref{sec:fcm}, répond à cette dernière problématique.
...
Cependant, différents problèmes peuvent survenir lors de la construction ou de la mise en \oe uvre de cet outil et doivent être pris en compte lors de la conception d'une méthodologie l'utilisant.

\subsection{Prise en compte du biais en intensité}
\label{subsec:art:bias}

Le biais en intensité affectant les IRM est un artéfact se présentant la plupart du temps comme une variation lente de l'intensité dans l'image.
...
Dans le cas de la segmentation anatomique adulte, cette hypothèse est suffisante pour effectuer la segmentation d'une IRM, mais nécessite l'intégration de modélisations complémentaires de manière à prendre en compte le biais (voir Figure~\ref{FIG:EX:FCM:BIAS}) et le bruit (voir Figure~\ref{FIG:EX:FCM:NOISE}).
...
Quelques approches différentes sont également présentées à la fin de cette section.
...
Une autre façon de modifier la mesure de similarité et de trouver des frontières entre les classes pertinentes est d'introduire la distance de Mahalanobis à la place de la distance euclidienne, ce qui revient à définir la mesure de similarité comme~\cite{Gustafson:CDC:1978, He:CMIG:2008} :
$$
...
Cette méthode se distingue par l'utilisation d'un principe proche de celui d'un atlas statistique, mais évite les problèmes d'utilisation que peuvent poser un atlas comme le recalage et la construction de l'atlas lui-même.
...
De plus, comme dit précédemment, ces algorithmes peuvent devenir très coûteux en temps de calcul à cause de l'exploration des voisinages.
...
Un autre illustration de ce genre d'approches est le travail de \cite{Weisenfeld:NeuroImage:2009}, qui parvient à une segmentation complète des structures cérébrales par l'utilisation d'une estimation non-paramétrique de la distribution d'intensités.
...
La méthode utilisée est la classification utilisant les méthodes bayésiennes avec une régularisation par champs de Markov.
...
Les images acquises \emph{in utero} présentent des difficultés supplémentaires, dues au temps d'acquisition plus faibles et aux conditions d'acquisition particulières (impossibilité d'immobiliser le patient, signal perturbé par les différents tissus de la mère, \emph{etc}.).
...
Des méthodes de reconstruction de volume ont également été définies afin d'obtenir des images isotropes à partir d'images de plus faibles résolutions et anisotropes~\cite{Rousseau:AR:2006, Kim:TMI:2010, Jiang:TMI:2007, Rousseau:MICCAI:2010}.
