...
Dans un deuxième temps, nous décrivons plus en détail les méthodes basées sur l'algorithme FCM (\emph{Fuzzy C-Means}), un algorithme de classification suffisamment souple pour intégrer de nombreuses connaissances pour la segmentation des images.
...
Cette théorie cherche à extraire le contenu d'une image à partir de mesures fondées sur les formes structures des objets contenus dans les images étudiées. 
...
Une autre catégorie de méthode est celle définie par un algorithme de segmentation appellé la ligne de partage des eaux (formalisé par \cite{Digabel:QAMSS:1978}).
...
Les champs de Markov se sont révélés particulièrement bien adapté à la segmentation des tissus cérébraux dans un environnement bruité, comme le montre les publications de \cite{Held:TMI:1997}, \cite{VanLeemput2:TMI:1999}, \cite{Zhang:TMI:2001} et \cite{Shattuck:NeuroImage:2001}.
...
La topologie d'un objet représente les caractéristiques qui doivent demeurer invariantes au cours de la transformation, c'est à dire conserver le même nombre de composantes connexes, de cavités et de tunnels.
...
Un premier exemple est l'article de \cite{Mangin:JMIV:1995} qui utilise les déformations homotopiques pour obtenir une cartographie du cortex cérébral, le but final étant l'étude des variabilité inter-individuelles des sillons.
