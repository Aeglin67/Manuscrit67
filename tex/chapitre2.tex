...
Cette déformation est effectuée par la minimisation d'une fonctionnelle se présentant comme la somme d'un terme d'attache aux donnée (donc lié à l'image) et d'un terme de régularisation (prenant en compte l'élasticité et la rigidité du contour).
Le principal problème des ces modèles sont leur forte dépendance à l'initialisation. 
En effet, le contour doit être déjà proche de la structure à segmenter et cela reste vrai malgré l'utilisation des \emph{gradient vector flow} (voir \cite{Xu:TIP:1998}) permettant une initialisation moins proche.
Un exemple de leur utilisation est l'article de \cite{McInerney:TMI:1999} qui emploit une formulation du contour particulière afin de mieux prendre en compte la complexité des structures.
...
Modèle implicite : les level-sets géométrique \cite{Casselles:NM:1993} et géodésique \cite{Casselles:IJCV:1997}. Très gourmand en temps de calcul, mise en place des algorithme \emph{fast marching} : \cite{Sethian:PNAS:1996}.
...
\cite{Mangin:MICCAI:1998} : Sélection de modes par une analyse multi-échelle de l'histogramme de l'image + opérateur morphologique pour extraire les différents tissus.
...
\cite{Park:IWSCA:2007} : Méthode automatique basée sur l'utilisation de plusieurs opérateurs : rejet du fond, normalisation (debiaisement), extraction du cerveau sur une coupe, extension aux autres coupes.
...
\cite{Jimenez-Alaniz:TMI:2006} : Estimation de la densité de probabilité par Mean-Shift et segmentation par a priori sur les tissus et carte de confiance des bords. Non-supervisée (Mean-Shift : cherche les modes (ou points de plus hautes densités) de la distribution)).
...
\cite{Marroquin:TMI:2002} : MAP avec Markov cachés. Utilisation d'un atlas pour calculé des \emph{a priori} pertinents et avoir une initialisation automatique.
...
\cite{Marroquin:TMI:2002} : Utilisation d'un atlas.
...
\cite{Zhou:TBE:2007} : Fuzzy Connectivness et atlas. A completer.

Multi-atlas : 

\cite{Heckemann:NeuroImage:2006} : Multi-atlas, décrit une méthode combinant recalage non-rigide et une méthode de fusion définie par \cite{Kittler:PAMI:1998} (définie quatre critères de décision : max, min, medium and vote majoritaire).

\cite{Aljabar:NeuroImage:2009} : Multi-atlas avec recalage. Large base d'atlas (environ 250 cas) donc grand temps de calcul du recalage. Sélection d'un nombre restreint d'atlas en fonction de leur similarité avec le sujet traité. Deux façon de faire cette sélection : directement par comparaison de l'intensité des images ou par la comparaison de méta-données telles que : age, pathologie, historique clinique, \ldots{} Choix de métrique également très important.

\cite{Rousseau:TMI:2011} : Propagation de labels à partir d'exemples. Introduction d'une recherche non-locale.
...
Plusieurs approches pour le bruit.
\begin{itemize}
