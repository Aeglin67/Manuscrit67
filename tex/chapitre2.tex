...
Elle représente une étape importante pour de nombreuses tâches, que ce soit dans un environnement clinique (par exemple le suivi des lésions provoquées par une maladie dégénérative type Parkinson ou Alzheimer) ou purement applicatif (par exemple : reconstruction du cortex en 3D, quantification des différentes structures du cerveau et évaluation de leur variabilité, \ldots).

Les méthodes de segmentation proposées doivent tenir des différentes caractéristiques et artefacts qui composent une IRM (voir la section \ref{intro:irm:carac}).
...
Cependant, ces modèles doivent être aussi proches que possible de la réalité, sous peine d'introduire de multiples erreurs dans la segmentation, ce qui pourrait provoquer une mauvaise interprétation du contenu de l'image et mener à de fausses conclusions dans le cadre d'une étude ou d'un suivi clinique.
...
Dans un premier temps, les grandes familles de méthodes sont identifiées et leur principale caractéristiques décrites.
Dans un deuxième temps, nous décrivons plus en détail les méthodes basées sur l'algorithme FCM (algorithme des C-moyennes floues ou \emph{Fuzzy C-Means} en anglais), un algorithme de classification suffisamment souple pour intégrer de nombreuses connaissances pour la segmentation des images.
Enfin, un état de l'art des techniques de segmentation utilisées dans le cadre de la maturation cérébrale est présenté.
En effet, même si on retrouve les grandes familles de la segmentation dans ce cadre, des approches néanmoins spécifiques ont du être développées pour répondre à cette problématique récente.
...
Cette déformation est effectuée par la minimisation d'une fonctionnelle se présentant comme la somme d'une force externe (le terme d'attache-aux-données lié au contenu de l'image) et d'une force interne (le terme de régularisation prenant en compte l'élasticité et la rigidité du contour).
Les principaux problèmes de ces modèles sont leur forte dépendance à l'initialisation et la difficulté de réaliser des changements de topologie (fusion ou séparation de labels). 
...
Ces modèles peuvent être utilisés comme une étape finale de la segmentation, destinée à corriger certains détails comme dans l'article de \cite{Stella-Atkins:TMI:1997}.
Un autre exemple de leur utilisation est l'article de \cite{McInerney:TMI:1999} qui emploie une formulation particulière du contour afin de mieux prendre en compte la complexité des structures.
...
Enfin, nous pouvons également citer les travaux de \cite{Wang:TBE:2009} utilisant un nouveau formalisme, appelé FVF (\emph{Fluid Vector Flow} en anglais), qui permet une meilleure segmentation de structures présentant une forte concavité.
...
L'article de \cite{Han:TPAMI:2003} définit un nouveau type de \emph{levels sets} permettant de respecter les contraintes topologiques du modèle initial.
Une application à la reconstruction de la surface corticale montre que l'ajout de ces contraintes permet de corriger des détails anatomiquement peu pertinents (tel que l'apparition de boucles, le cortex étant une structure très circonvoluée).

Les articles de \cite{Yang:TMI:2004} et \cite{Duncan:NeuroImage:2004} sont basés sur l'évolution de \emph{level-sets} concurrents \emph{via} une estimation du maximum \emph{a posteriori} (MAP) prenant en compte la forme des objets à segmenter et leurs relations de voisinage.
...
Dans le même esprit d'utiliser plusieurs sources d'information pour guider le contour, l'article de \cite{Ciofolo:MIA:2009} définit une méthodologie décidant de la direction de propagation à partir d'un contrôleur flou prenant en compte un \emph{a priori} donné par un atlas anatomique, l'intensité des voxels, la position du contour dans l'image et par rapport à d'autres objets.
...
Toutes ces méthodes restent néanmoins trop dépendantes de l'initialisation du contour et des pré-traitements à effectuer.
...
Une élément structurant, caractérisant les propriétés recherchées, parcourent alors l'image et agit à la manière d'un filtre et doit donc être choisi en fonction de l'application recherchée.
...
Ce genre de méthodologie a été utilisée pour extraire le cerveau du reste de l'image.
...
Les principales catégories sont les algorithmes non-paramétriques (supervisés ou non), les approches statistiques, l'algorithme des K-moyennes et l'algorithme des C-moyennes floues (FCM).
...
Plusieurs types d'algorithmes existent, consistant soit à estimer la fonction de densité à partir d'échantillons, qui si ils sont satisfaisant deviennent représentatifs de cette fonction, soit d'estimer directement la probabilité \emph{a posteriori} de la distribution.
...
De manière générale, l'information prise en compte est l'intensité des voxels, mais également leurs coordonnées spatiales, ce qui conduit à une sur-segmentation de l'image.
...
Une approche locale a également été introduite par \cite{Kovacevic:NeuroImage:2002} qui modélise l'histogramme du volume intracrânien par une mixture de quatre gaussiennes aussi bien au niveau locale que globale pour apporter une réponse au problème des inhomogénéités en intensité.
...
En conclusion, nous constatons que des modèles locaux de segmentation émergent (voir \cite{Richard:AIM:2004} et \cite{Scherrer:TMI:2009}), l'enjeu étant de définir les règles de coopération entre ces différentes modèles de manière à obtenir une segmentation finale cohérente.

FIXME : A ajouter, \cite{Cardoso:NeuroImage:2011} qui fait une segmentation un affinement de la segmentation du cortex en changeant localement l'\emph{a priori} de l'atlas, une modélisation explicite du volume partiel et un changement locale du poids des MRF.
...
\begin{equation}
J_{K-\text{Means}} = \sum_{j=1}^{N} \sum_{k=1}^{K} \lVert \mathbf{y}_j - \mathbf{v}_k \rVert^{2}_{2},
\end{equation}
où $\mathbf{y}_j$ représente l'intensité du voxel $j$ et $\mathbf{v}_k$ le centroïde de la classe $k$.
L'algorithme démarre d'une position initiale et alterne une étape d'appariement des données aux classes et une étape de mise à jour des centroïdes.
...
Cependant, il est sensible à l'initialisation, ne tient pas compte d'un environnement bruité et étant donné le fort effet de volume partiel présent dans les IRM, l'appartenance binaire à une classe peut poser problème.
Aujourd'hui, l'algorithme des $C$-Moyennes floues (FCM), décrit à la section \ref{sec:fcm}, lui est préféré.
...
La topologie est un vaste domaine des mathématiques, fondée par Euler grâce à la solution qu'il a apporté au problème des ponts de K\"{o}nigsberg \cite{Euler:CASP:1741}.
...
Cependant, l'application dans le domaine numérique (donc discrétisé), n'est pas immédiate car une transformation voulue comme continue peut rompre ces invariants topologiques en fonction de la définition de la connexité et de l'échantillonnage.
...
Une autre approche intéressante est celle de \cite{MacDonald:NeuroImage:2000}, qui segmente le cortex à l'aide de deux level-sets concurrents tout en imposant comme contraintes que ces deux surfaces ne peuvent pas se croiser, ainsi qu'une distance maximum entre ces deux surfaces (ce qui revient à imposer une épaisseur maximum au cortex).

La méthode de \cite{Han:TMI:2002} se situe plus dans une approche de correction de la topologie.
A partir d'une segmentation du cortex, le but est d'obtenir une surface cohérente topologiquement (c'est à dire sans tunnels, ni cavités qu'une pré-segmentation a toute les chances d'avoir créés).
...
Chaque structures est extraite successivement selon des contraintes prédéfinies en fonction de connaissances anatomiques.
...
La suite de ces travaux (voir \cite{Bazin:MIA:2008}) montre une amélioration de l'algorithme par l'introduction d'un atlas statistique et par l'ajout de propriétés homéomorphiques permettant de prendre ainsi en compte plus facilement les contraintes topologiques dans un environnement multi-objet.

Une dernière méthode à citer est celle de \cite{Miri:ICISP:2008} qui effectue une segmentation des tissus cérébraux à partir de quatre sphères concentriques évoluant par échange de points simples.
...
Il peut être utilisé seul, ou en collaboration avec d'autres outils permettant de prendre en compte le bruit présent dans l'image.
...
L'article de \cite{Ashburner:NeuroImage:2005} présente une méthode reposant presque complètement sur le recalage d'un atlas et introduisant un \emph{a priori} sur les tissus combinant l'\emph{a priori} de l'atlas pondéré par la quantité de tissus présent dans un petit volume permettant une bonne résistance au bruit dans les régions où un tissu est prépondérant.
Un autre exemple est donné par l'article de \cite{Zhou:TBE:2007} qui utilise des notions de connectivité floue combinées à un atlas pour obtenir une segmentation finale.
...
Cependant, l'utilisateur doit être conscient des différents problèmes qui peuvent survenir, que ce soit lors de sa construction, ou lors de son utilisation surtout lors de l'étape critique du recalage.
...
Il peut être du à l'imperfection de l'imageur (défaut de l'antenne de réception, hétérogénéité du champs magnétique statique, \ldots{}) ou à des causes biologiques (composition différente d'un même tissu à différents endroits du cerveau, ou effet de susceptibilité magnétique).
...
Les approches séquentielles ont historiquement d'abord reposé sur des filtrage spatiaux ou homomorphiques \cite{Johnston:TMI:1996} considérant que les basses fréquences représentent le biais tandis que les hautes fréquence représentent les détails anatomiques.
...
L'article de \cite{Mangin:MMBIA:2000} utilise cette approche avec une contrainte de lissage sur le champs de biais et une contrainte de similarité entre la moyenne de l'image originale et celle de l'image restorée.
Une autre technique consiste à réaliser un ajustement de surface en modélisant le champs de biais par des splines ou par un polynôme, comme dans \cite{Styner:TMI:2000}, la correction se faisant alors voxel à voxel par division de l'image originale par le champs calculé.
...
Le plus courant consiste à transformer les intensités dans le domaine logarithmique, transformant le champs de biais généralement considéré comme multiplicatif, en un artefact additif donnant le modèle suivant : 
\begin{equation}
...
Ces hypothèses sont notamment : la manière dont le bruit et le biais interagissent, la nature multiplicative du biais, la nature lente des variations en intensité, la modélisation de ces variations (polynômes, splines (\cite{Marroquin:TMI:2002,Liew:TMI:2003}), \ldots), la définition d'un champs de biais unique ou d'un champs particulier pour chaque tissu (comme dans l'article de \cite{Marroquin:TMI:2002}), \ldots
...
\begin{enumerate}
        \item Initialisation des centroïdes (peut-être faite par un algorithme $k$-means, grâce à un atlas ou aléatoirement).
...
A notre connaissance, aucune méthode ne permet de déterminer de façon fiable le nombre de classes de manière automatique.
...
Dans le cas de la segmentation anatomique adulte, cette hypothèse est suffisante pour effectuée la segmentation d'une IRM, mais nécessite l'intégration de modélisations complémentaires de manière à prendre en compte le biais et le bruit.
...
Deux grandes voies ont été suivies, la première utilisant des opérateurs noyaux de manière à projeter les données dans un autre espace où la segmentation serait plus aisée et la deuxième utilisant la distance de Mahalanobis (utile en cas d'utilisation de données multi-spectrales).
Des voies spécifiques ont également été suivies et sont présentées à la fin de cette section.

\subsubsection{Projection dans un espace de dimension supérieure}

Concernant l'utilisation des opérateurs noyaux, un état de l'art général de leur utilisation en clustering est fournit par~\cite{Filippone:PR:2008}.
...
Afin de régler au mieux les paramètres contrôlant les poids respectifs de ces deux termes, une optimisation par un réseau de neurones est mise en place.
...
On rappelle que le signal reçu est modélisé comme le produit du \og vrai \fg{} signal et d'un champ de biais, supposé varier lentement le long du support de l'image.
...
Ils introduisent un modèle appelé un clustering des intensités local et cohérent (en anglais : \emph{coherent local intensity clustering} ou \emph{CLIC}) destiné à prendre en compte la similarité des intensités d'un voisinage.
...
\begin{equation}
J_{CLIC} = \sum_{j \in \Omega} \sum_{k = 1}^{C}u_{jk}^q \sum_{l \in \Omega} K(\mathbf{x}_j - \mathbf{x}_l) \lVert \mathbf{y}_{j} - b_{l}\mathbf{v}_{k} \rVert^2.
\end{equation}
...
Les travaux de~\cite{Szilagyi:AICIEEE:2003} parviennent à ce but en définissant une image intermédiaire $\zeta$ de la manière suivante : 
\begin{equation}
...
\end{equation}
Les avantages de cette méthodologie sont donc un traitement accéléré et une procédure ramenée à l'algorithme FCM classique, l'image d'entrée étant l'image intermédiaire qui prends en compte l'attache-aux-données et le terme de régularisation directement.
...
Cet algorithme est appelé le KFCM\_S et la fonction d'énergie à minimiser devient : 
\begin{equation}
...
Les deux méthodes précédentes, tout comme les méthodes de~\cite{Ahmed:TMI:2002} et \cite{Pham:CVIU:2001}, ont le désavantage de reposer sur un paramètre $\beta$ contrôlant les poids respectifs entre les termes d'attache aux données et le terme de régularisation.
...
Une dernière méthode, appelée \emph{Fuzzy Local Information C-Means} (FLICM) est à mettre en valeur. 
...
\end{equation}
La principale remarque est qu'il n'y a plus besoin d'un paramètre réglant la balance entre l'attache aux données et la régularisation.
...
Une première étude concernant les tissus cérébraux chez les enfants a été effectuée par~\cite{Matsuzawa:CC:2001} et comprends des patients âgés de 1 mois à 10 ans.
Il s'agit d'une étude volumétrique cherchant à différencier le LCR, la matière grise et la matière blanche.
Cependant, cette étude ne prend pas en compte un élément capital, qui est la myélinisation de la matière blanche au cours du temps.
...
Afin de palier au défaut de l'article de \cite{Matsuzawa:CC:2001} qui ne prend pas en compte la myélinisation de la matière blanche au cours du temps, il a introduit une séparation de la matière blanche myélinisée et non myélinisée.
...
L'estimation initiale de la distribution en intensité est réalisé grâce à un seuillage d'un atlas statistique préalablement recalé sur le cas à traiter.
...
Les prototypes sont alors mis à jour et la segmentation réestimé jusqu'à ce qu'aucun changement ne soit observé dans le résultat final.

Les approches par classification utilisent toutes un algorithme EM, avec cependant quelques variantes.
...
En résumé, la segmentation du cortex est obtenue par une restriction de la zone d'intêret où la segmentation est effectuée puis par l'utilisation d'\emph{a priori} markoviens locaux, permettant une segmentation plus fiable du cortex.

L'approche de~\cite{Merisaari:JNM:2009} est intéressante car elle ne repose que sur les données de l'image pour effectuer la segmentation (séparation de la LCR et du cerveau dans ce cas).
Premièrement, une segmentation par ligne de partage des eaux est réalisée, conduisant à un grand nombre de régions. 
Dans un deuxième temps, la moyenne de chaque région est calculé, ces moyennes permettant de classer les différentes régions selon un modèle de mixture de gaussienne.
...
Toutes ont besoin d'utiliser un atlas, soit comme outils statistique afin de bénéficier de l'\emph{a priori} spatial qu'il apporte, soit pour aider à la sélection d'échantillons dans l'image, soit pour exclure une partie du cerveau de la zone d'intêret pour la segmentation.
...
L'article de~\cite{Gui:ISBI:2011} montre qu'il est possible d'obtenir une segmentation en ne se basant que sur les données de l'images par une succession d'opérations connues, mais mise en \oe uvre dans l'ordre nécessaire à l'obtention d'une segmentation complète.
...
Cette méthode semble performante mais elle dépend largement de la détection des noyaux gris.
...
Des méthodes de reconstruction de volume ont également été définies afin d'obtenir des images isotropes à partir d'images de plus faibles résolutions et anisotropes~\cite{Kim:TMI:2010,Rousseau:AR:2006}.
...
A partir de cette localisation des yeux, la coupe sagittale passant au milieu du cerveau est reconstruite (l'orientation du sujet n'est pas connue) et une première segmentation du volume intracrânien est effectuée dans cette coupe.
...
La première fois permet de labélisé tous les voxels voisins du fond de l'image comme étant du LCR, la deuxième permet de segmenter le cortex en imposant un critère d'épaisseur et la troisième permet de séparer les noyaux gris du cortex et de classer les voxels de transition encore présent.
Cette méthode est intéressante car elle introduit des contraintes anatomiques, cependant la phase de régularisation déconnectée des données n'est pas sans poser quelques problèmes car rien ne garantit que la segmentation peut se corriger automatiquement si elle n'est pas guidée par les données de l'image.
...
Dans ce chapitre, nous avons proposé un état des lieux de la segmentation des structures cérébrales en IRM.
De nombreuses méthodes ont été définies, chacune ayant ses qualités et ses défauts.
Par exemple, les modèles déformables se révèlent efficaces lorsqu'une structures particulière (le cortex, les ventricules, les noyaux gris, \ldots) est recherchée.
Les méthodes de classification sont quand à elles plus efficaces dans le cadre de la segmentation des principaux tissus cérébraux (LCR, matière grise et matière blanche).

Parmi les différentes méthodes de classification, la méthode FCM a retenu notre attention.
% Concernant la segmentation par FCM, les différentes méthodologies montrent de bonnes performances des différents algorithmes dévellopés en classification des tissus.
Son avantage est qu'elle procure une réponse native au problème du volume partiel en fournissant des fonctions d'appartenance à chaque tissus, tout en étant facilement extensible de manière à inclure de nouvelles fonctionnalités.
...
Cette connaissance prend généralement la forme d'un atlas, mais peut prendre aussi la forme d'un ajustement des coefficients régissant le comportement des champs de Markov.
L'utilisation de connaissances anatomiques génériques commence tout juste à être exploitée, que ce soit en modélisant le cerveau en couches, ou en utilisant ces \emph{a priori} afin d'obtenir une segmentation progressive des structures cérébrales.
Ce type d'approche semble prometteur car il permet d'inclure des connaissances universelles et ne nécessite pas la délicate phase de construction et d'utilisation d'un atlas statistique.
...
Le deuxième est une contribution à la segmentation du cortex d'un cerveau non mature (IRM \emph{in vivo} au cours de la grossesse) par l'utilisation de connaissance anatomiques génériques, permettant ainsi de se passer d'un atlas statistique (voir le chapitre \ref{chap:foetale}).
