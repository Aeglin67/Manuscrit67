...
Cependant, ces modèles doivent être aussi proche que possible de la réalité, sous peine d'introduire de multiples erreurs dans la segmentation, ce qui pourrait provoquer une mauvaise interprétation du contenu de l'image et mener à de fausse conclusion dans le cadre d'une étude ou d'un suivi clinique.
...
En effet, même si on retrouve les grandes familles de la segmentation dans ce cadre, des approches néanmoins spécifiques ont du être dévelloppées pour répondre à ces problématiques récentes.
...
Les principaux problèmes des ces modèles sont leur forte dépendance à l'initialisation et la difficulté de réaliser des changements de topologie (fusion ou sépration de labels). 
...
Ces modèles peuvent être utilisé comme une étape finale de la segmentation, destinée à corriger certains détails comme dans l'article de \cite{Stella-Atkins:TMI:1997}.
...
Plus récemment, l'article de \cite{Colliot:PR:2006}, définissant la force externe comme une combinaison du terme d'attache-aux-données et d'une information spatiale concernant la position des différents objets entre eux.
Cette information spatiale est formalisée sous la forme de contraintes de localisation (angle et distance) exprimée de façon floue par rapport à une structure de référence.
Cette méthodologie a été appliqué à la segmentation des noyaux gris autour des ventricules.
Enfin, nous pouvons également citer les travaux de \cite{Wang:TBE:2009} utilisant un nouveau formalisme, appellée FVF (\emph{Fluid Vector Flow} en anglais), qui permet une meilleure segmentation de structure présentant une forte concavité.
