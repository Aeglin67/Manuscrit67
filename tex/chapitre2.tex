...
Les méthodes de segmentation proposées doivent tenir compte des différentes caractéristiques et artefacts qui affectent une image IRM (voir la section \ref{intro:irm:carac}).
Elles peuvent intégrer une modélisation \emph{a priori} sur le bruit, le biais, la localisation et les caractéristiques des tissus cérébraux.
...
Les modèles explicites ou \emph{snakes} définis par \cite{Kass:IJCV:1988} s'appuient sur la déformation itérative d'un contour paramétrique.
...
Formellement, l'énergie de l'hypersurface $\mathcal{C}$ est écrite généralement de la manière suivante : 
\begin{equation}
J(\mathcal{C}) = \underbrace{\underbrace{\alpha \int \lvert \frac{\partial \mathcal{C}(q)}{\partial q} \rvert^{2} dq}_{\mbox{\'Elasticité}} + \underbrace{\beta \int \lvert \frac{\partial^{2} \mathcal{C}(q)}{\partial q^2} \rvert^{2} dq}_{\mbox{Rigidité}}}_{\mbox{\'Energie interne}} + \underbrace{\lambda \int g(\lvert \nabla I (\mathcal{C}(q)) \rvert) dq}_{\mbox{\'Energie externe}},
\end{equation}
où $q$ est la coordonnée relative à l'hypersurface, $\alpha$, $\beta$ et $\lambda$ sont des coefficients réels positifs, $\nabla I$ est le gradient de l'image $I$ et $g$ est une fonction strictement décroissante destiné à minimiser le terme d'énergie externe si l'hypersurface $\mathcal{C}$ est sur les zones de plus fort gradient.
...
Plusieurs techniques ont été utilisées pour améliorer les contours paramétriques, telles que les \emph{gradient vector flow}~\cite{Xu:TIP:1998} permettant une moins forte dépendance à l'initialisation, où les \emph{fluid vector flow}~\cite{Wang:TBE:2009} prenant mieux en compte la concavité des objets.
...
Il parvient à cet objectif en plongeant le contour dans une décomposition affine cellulaire de l'image, qui divise cette dernière en une collection de polygones convexes.
...
Cependant, la simulation de cette propagation est très couteuse en temps de calcul, menant à la définition d'algorithmes \emph{fast marching} \cite{Sethian:PNAS:1996}.
...
Les images peu contrastées ou présentant un biais en intensité peuvent devenir un problème, d'autant que la correction de biais tend à réduire le contraste dans certaines zones du cerveau~\cite{Colliot:PR:2006}.
...
Un élément structurant, caractérisant les propriétés recherchées, parcoure alors l'image et agit à la manière d'un filtre et doit donc être choisi en fonction de l'application recherchée.
...
L'utilisateur à le choix des seuils et des critères de convergence pour la segmentation.
...
Ils sélectionnent des modes par une analyse multi-échelle de l'histogramme de l'image et utilise des opérateurs morphologiques pour l'extraction les différents tissus.
...
Cette segmentation est contrainte par des critères topologiques (position des tissus les uns par rapport aux autres, \emph{etc}.).
...
\end{equation}
L'objectif est de donner un label $x_j$ à chaque voxel $j$ avec $x_j \in \{c_1, c_2, \ldots, c_k\}$.
...
La topologie d'un objet représente les caractéristiques qui doivent demeurer invariantes au cours de la transformation, c'est-à-dire que l'objet final doit, en particulier dans le cas 3D, comporter le même nombre de composantes connexes, de cavités et de tunnels que l'objet initial.
...
La notion au c\oe ur de cette nouvelle topologie est celle du \emph{point simple}, qui représente un voxel pouvant être librement labélisé comme faisant partie de l'objet ou n'en faisant pas partie sans en \og changer la topologie \fg{} (par exemple le type d'homotopie).
...
Une méthode récente \cite{Rousseau:TMI:2011} tente de contourner le problème du recalage en comparant des patches (petit voisinage autour d'un voxel) issus de l'image originale et de l'atlas anatomique.
La comparaison de ces patchs conduit à un vote pondéré par la similarité entre ces patches.
Seul un recalage affine est alors nécessaire pour amener les deux images dans les mêmes coordonnées.

De manière générale, l'utilisation d'un atlas se distingue grâce à l'\emph{a priori} anatomique et statistique qu'il permet d'introduire dans la segmentation.
...
Le biais en intensité affectant les IRM est un artefact se présentant la plupart du temps comme une variation lente de l'intensité dans l'image.
Il peut être dû à l'imperfection de l'imageur (défaut de l'antenne de réception, hétérogénéité du champs magnétique statique, \emph{etc}.) ou à des causes biologiques (composition différente d'un même tissu à différents endroits du cerveau, ou effet de susceptibilité magnétique).

Plusieurs approches ont été envisagées pour la prise en compte du biais en intensité.
Deux grandes familles de méthodes se dégagent : les approches séquentielles (effectuant une correction du biais avant la segmentation) et les approches couplées (le biais est pris en compte directement dans le processus de segmentation).
...
Elle contient $N$ voxels, $\mathbf{x}_{j}$ et $\mathbf{y}_{j}$ représentent respectivement les coordonnées spatiales et les valeurs du voxel $j$ (plusieurs valeurs possibles dans le cas où plusieurs modalités d'images sont utilisées).
...
Un exemple de l'utilisation de ces opérateurs est le \emph{Support Vector Machine} (SVM) \cite{Cortes:ML:1995} qui est un algorithme de classification supervisé.
...
L'image est divisé en sous-volumes avec un recouvrement et un modèle local est estimé pour chaque sous-volume.
...
\end{equation}
La forte présence d'autres classes dans le voisinage autour du voxel $j$ a pour conséquence une diminution du dénominateur, diminuant d'autant l'appartenance à la classe courante.
...
L'analyse de l'expression des fonctions d'appartenance est donnée :
\begin{equation}
...
\end{equation}
montre que l'ajout de ce terme de régularisation entraîne une diminution du dénominateur si l'intensité des voxels voisins est trop dissemblable de la valeur du centroïde.
...
\subsubsection{Pondération entre l'attache aux données et la régularisation}
...
Une première étude utilisant une approche bayésienne avec régularisation par champs de Markov, qui est également l'une des première conduite sur les enfants, a été effectuée par~\cite{Matsuzawa:CC:2001} et comprend des cas âgés de 1 mois à 10 ans.
...
De plus, le recalage d'un modèle des éléments subcorticaux sur le cas étudié permet d'ignorer cette partie du volume intracrânien, restraignant la zone d'intérêt ou est effectuée la segmentation et permettant de ne pas confondre les noyaux gris et le cortex.
...
L'article de~\cite{Gui:ISBI:2011} montre qu'il est possible d'obtenir une segmentation en ne se basant que sur les données de l'images par une succession d'opérations connues, mais mises en \oe uvre de manière à obtenir une segmentation complète.
...
Un premier exemple de cette approche est la méthode de~\cite{Claude:TBE:2004} (également première tentative de segmentation des structures cérébrales \emph{in vivo}), consistant en une segmentation semi-automatique.
...
Cette méthodologie utilise des opérateurs connus et n'est pas complètement automatique, son intérêt étant la fa\c on dont les différents opérateurs sont mis en \oe uvre pour obtenir la segmentation.
...
La première fois permet de labéliser tous les voxels voisins du fond de l'image comme du LCR, la deuxième permet de segmenter le cortex en imposant un critère d'épaisseur tandis que la troisième permet de séparer les noyaux gris du cortex et de classer les voxels de transition encore présents.
Cette méthode se distingue par l'introduction de contraintes anatomiques, telles que l'épaisseur du cortex et de quelle façon les tissus sont positionnés les uns par rapport aux autres.
