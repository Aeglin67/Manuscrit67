...
Cette évolution est inspirée des travaux en propagations des ondes de \cite{Osher:JCP:1988}, formalisant l'équation d'évolution sous forme d'équations aux dérivées partielles.
...
L'article de \cite{Baillard:MIA:2001} propose une segmentation du cerveau en deux étapes.
...
Une application à la reconstruction de la surface corticale montre que l'ajout de ces contraintes permet de corriger des détails afin de respecter l'anatomie du cortex (tels que l'apparition de boucles, le cortex étant une structure très circonvoluée).

\cite{Zeng:TMI:1999} définit une méthode de segmentation du cortex par deux lignes de niveaux concurrentes, l'avantage étant qu'elles permettent de définir explicitement une frontière LCR/matière grise et matière grise/matière blanche, tout en gardant une distance minimale entre ces deux surfaces.
...
Les travaux de \cite{Mangin:MICCAI:1998} présentent quant à eux une méthode complètement automatique de segmentation des structures cérébrales.
...
Enfin, les travaux de \cite{Chiverton:CBM:2007} présentent également une méthode automatique d'extraction du cerveau basée sur la morphologie mathématique.
...
Une autre catégorie de méthodes est celle définie à partir d'un algorithme de segmentation appelé la ligne de partage des eaux (formalisé par \cite{Vincent:PAMI:1991}).
...
\cite{Warfield:MIA:2000} montre une approche utilisant le kPPV avec un ensemble d'apprentissage défini par un expert.
...
\cite{Mayer:TMI:2009} définit une approche reposant sur une première étape toujours fondée sur un \emph{Mean-Shift} avec la prise en compte de l'intensité et des coordonnées spatiales.
...
L'algorithme ICM (\emph{Iterated Conditional Modes}) de \cite{Besag:JRSS:1986}, cherchant le MAP de la distribution, est également utilisé.

Parmi le grand nombre d'articles utilisant cette technique, nous pouvons citer tout d'abord les articles de~\cite{VanLeemput1:TMI:1999} et \cite{Ashburner:NeuroImage:2000}, qui ajoutent une correction des inhomogénéités en intensité.
Celui de \cite{Dugas:MICCAI:2004} ajoute une prise en compte du volume partiel, et celui de \cite{Ait:MICCAI:2005} remplace l'étape de maximisation de la vraisemblance par un estimateur de vraisemblance tamisé.
...
Les travaux de \cite{Bricq:MIA:2008} utilisent une régularisation par chaînes de Markov, où le voisinage est pris en compte grâce à un parcours fractal de l'image.
...
Une autre approche est celle de \cite{MacDonald:NeuroImage:2000}, qui segmente le cortex à l'aide de deux lignes de niveaux concurrentes tout en imposant comme contraintes que ces deux surfaces ne peuvent pas se croiser, ainsi qu'une distance maximum entre ces deux surfaces (ce qui revient à imposer une épaisseur maximum au cortex).
...
Nous pouvons également citer les travaux \cite{Bazin:TMI:2007} qui utilisent un algorithme de type FCM conjugué à un modèle anatomique prenant en compte les contraintes topologiques des différentes structures.
...
\cite{Ashburner:NeuroImage:2005} présente une méthode reposant en majeure partie sur le recalage d'un atlas et introduisant un \emph{a priori} sur les tissus combinant l'\emph{a priori} de l'atlas pondéré par la quantité de tissus présente dans un petit volume permettant une bonne robustesse au bruit dans les régions où un tissu est prépondérant.
...
Il peut être dû à l'imperfection de l'imageur (défaut de l'antenne de réception, hétérogénéité du champ magnétique statique, \emph{etc}.) ou à des causes biologiques (composition différente d'un même tissu à différents endroits du cerveau, ou effet de susceptibilité magnétique).
...
\cite{Mangin:MMBIA:2000} utilise cette approche avec une contrainte de lissage sur le champ de biais et une contrainte de similarité entre la moyenne de l'image originale et celle de l'image restaurée.
...
\begin{figure*}[!htb]
\centering
\subfigure[]{\includegraphics[width=34mm]{eps/chapitre2/original.eps}}
\hspace{1mm}
\subfigure[]{\includegraphics[width=34mm]{eps/chapitre2/gm_classic.eps}}
\hspace{1mm}
\subfigure[]{\includegraphics[width=34mm]{eps/chapitre2/wm_classic.eps}}
\hspace{1mm}
\subfigure[]{\includegraphics[width=34mm]{eps/chapitre2/csf_classic.eps}}
...
\centering
\subfigure[]{\includegraphics[width=34mm]{eps/chapitre2/original_noise.eps}}
\hspace{1mm}
\subfigure[]{\includegraphics[width=34mm]{eps/chapitre2/gm_noise.eps}}
\hspace{1mm}
\subfigure[]{\includegraphics[width=34mm]{eps/chapitre2/wm_noise.eps}}
\hspace{1mm}
\subfigure[]{\includegraphics[width=34mm]{eps/chapitre2/csf_noise.eps}}
...
\centering
\subfigure[]{\includegraphics[width=34mm]{eps/chapitre2/original_bias.eps}}
\hspace{1mm}
\subfigure[]{\includegraphics[width=34mm]{eps/chapitre2/gm_bias.eps}}
\hspace{1mm}
\subfigure[]{\includegraphics[width=34mm]{eps/chapitre2/wm_bias.eps}}
\hspace{1mm}
\subfigure[]{\includegraphics[width=34mm]{eps/chapitre2/csf_bias.eps}}
...
La première définition de l'algorithme FCM est donnée par \cite{Zadeh:IC:1965} et s'appuie sur la notion d'ensembles flous.
Très vite, cet algorithme a trouvé des applications, notamment dans le cadre médical, comme en témoignent les publications de \cite{Adey:IJN:1972}, \cite{Kalmanson:AJC:1975} et \cite{Bezdek:NCC:1976}.
...
Concernant l'utilisation des opérateurs noyaux, un état de l'art général de leur utilisation en partionnement est fourni par~\cite{Filippone:PR:2008}.
...
Les formulations décrites précédemment cherchent à obtenir une séparation fiable des données par l'utilisation d'un espace plus approprié, mais ne prennent pas en compte la présence de bruit dans l'image.
...
Tout d'abord, un filtre de diffusion est appliqué plusieurs fois à l'image à traiter, ce qui a pour effet d'obtenir une image avec un niveau de détails très réduit.
...
Nous pouvons cependant citer \cite{Zhu:NeuroImage:2003} et sa méthode FCM multicontexte.
...
Les fonctions d'appartenance sont calculée selon la formule suivante :
\begin{equation}
...
\begin{equation}
S^{g}_{jl} = \exp \left( \frac{-\lVert \mathbf{y}_j - \mathbf{y}_l \rVert^{2}}{\lambda_g \cdotp (\sigma^{g}_{i})^{2}} \right),
\end{equation}
...
Une première étude utilisant une approche bayésienne avec régularisation par champs de Markov, qui est également l'une des première conduites sur les enfants, a été effectuée par~\cite{Matsuzawa:CC:2001} et comprend des cas âgés de 1 mois à 10 ans.
