...
De manière générale, l'information prise en compte est l'intensité des voxels, mais également leurs coordonnées spatiales dans l'image, ce qui conduit à une sur-segmentation de l'image.
...
Les champs de Markov se sont révélés comme particulièrement bien adapté à la segmentation des tissus cérébraux dans un environnement bruité, comme le montre les publications de \cite{Held:TMI:1997}, \cite{VanLeemput2:TMI:1999}, \cite{Zhang:TMI:2001} et \cite{Shattuck:NeuroImage:2001}.
...
Un premier exemple est l'article de \cite{Mangin:JMIV:1995} qui utilise les déformations homotopiques pour obtenir une cartographie du cortex cérébral, le but final de l'étude étant l'étude des variabilité inter-individuelles des sillons.
...
Les paramètres à fixer pour l'algorithme FCM sont donc le nombre de classes $C$ et le choix de la fonction d'appartenance $D^2(\mathbf{y}_j, \mathbf{v}_k)$.
A notre connaissance, aucune méthode ne permet de déterminer de façon fiable le nombre de classes de manière automatique.
La question de la fonction d'appartenance sera discuté ultérieurement.
...
\end{equation}
De plus, afin de ne pas explorer l'image plusieurs fois au cours des itérations de l'algorithme, un paramètre $\gamma_{p}$ est introduit indiquant le nombre de voxels de l'image à une intensité $p$ donnée (par conséquent, on a $\sum_{p = 0}^{p_{max}} \gamma_{p} = N$).
...
\subsubsection{Détermination automatique de la balance entre attache-aux-données et régularisation}
...
Les méthodes de segmentation dans les cas post-nataux se regroupe en deux grandes familles, qui sont l'estimation de la distribution en intensité des différents tissus à partir d'exemple issu de l'image et la classification par un algorithme EM avec potentiellement une régularisation par champs de Markov.

Un premier exemple d'estimation de la distribution en intensité par des exemples est l'article de \cite{Prastawa:MIA:2005}.
Afin de palier à le défaut de l'article de \cite{Matsuzawa:CC:2001} qui ne prend pas en compte la myélinisation de la matière blanche au cours du temps, ils ont introduit une séparation de la matière blanche myélinisée et non myélinisée.
...
Un autre article illustrant ce genre d'approche est celui de \cite{Weisenfeld:NeuroImage:2009}, qui parvient à une segmentation complète des structures cérébrales par l'utilisation d'un estimation non-paramétrique de la distribution d'intensité.
...
Les prototypes sont alors mis à jour et la segmentation réestimé jusqu'à ce qu'aucun changement ne soit observé dans la résultat final.

Les approches par classification utilisent toute un algorithme EM, avec cependant quelques variantes.
...
Dans un deuxième temps, chaque région est labélisée selon le modèle des mixtures de gaussienne en fonction de la moyenne de ses intensités.
...
La seule méthode n'utilisant pas d'atlas est celle de \cite{Merisaari:JNM:2009}, mais ce dernier ne cherche qu'à séparer le cerveau du LCR.
...
Cependant, cette segmentation n'est réalisé que sur une coupe sagittale et nécessite une importante implication de l'expert pour délimiter la fosse postérieure du crâne.
