...
Ce chapitre décrit une méthode de segmentation du cortex en IRM f\oe tale.
...
En effet, la distribution d'une IRM adulte révèle trois pics distincts correspondant respectivement au LCR, et aux matières grise et blanche.
...
Rappelons tout d'abord que la segmentation par l'algorithme $k$-means s'apparente à la minimisation de la fonction coût suivante : 
\begin{equation}
...
Ces centroïdes locaux sont calculé de la façon suivante (voir la figure \ref{FIG:BIAS}) : 
\begin{enumerate}
...
Néanmoins, les contraintes géométriques sont conservées (par exemple, un label complètement inclue dans un autre le restera même si il est divisé en plusieurs composantes).
...
Cela permet de définir la frontière entre le LCR externe et la cerveau, permettant ainsi de construire une zone d'intêret incluant le cortex.
...
Les expériences sont menées sur huit cas acquis avec une machine IRM de 1.5 Tesla (Magnetom Avento, Siemens, Germany Erlangen) utilisant des spin d'écho ultra-rapide.
...
L'âge des différents f\oe tus va de 27 à 32 semaines d'aménorrhée (SA). 
...
La dimension des voxels est de $0.742\times0.742\times0.742$.
