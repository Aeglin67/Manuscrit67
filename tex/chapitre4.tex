...
La différence la plus notable est l'inversion des contrastes entre la matière blanche et la matière grise par rapport à une image adulte (le cortex apparait plus foncé que la matière blanche dans  le cas adulte et plus clair dans le cas d'un f\oe tus si l'image est pondérée T1).
Dans le cadre d'une classification, l'effet de volume partiel (présence de plusieurs tissus dans un même voxel conduisant à une altération du signal) propre aux IRM conduit alors à des artefacts comme la présence d'une fine bande de voxels classés comme matière blanche entre le LCR et le cortex, ce qui est anatomiquement abérrant (voir la figure~\ref{FIG:DISTRIB}).
...
Une analyse de différentes zones d'intêrets construites autour du cortex révèle des distributions en intensité des différents tissus bien plus favorable à une opération de partitionnement (par $k$-means ou par FCM). 
Cependant, le recouvrement de ces distributions reste important, laissant penser que l'information d'intensité seule n'est pas suffisante, et le problème posé par l'effet de volume partiel (apparation de voxels classés comme matière blanche entre le LCR et le cortex) n'est pas résolu.
...
La validation est menée dans un premier temps sur des images \og brutes \fg{} (c'est à dire non reconstruites), puis sur des volumes reconstruits.
...
Elles sont reconstruites en suivant la méthode de~\cite{Rousseau:AR:2006} à partir de trois acquisitions respectivement axiale, coronale et sagitale.
...
\subsection{Segmentation de volumes recontruits par l'apport de la super-résolution}
...
Cette section présente des résultats préliminaires issues de la segmentation de volumes reconstruits selon la méthode de \cite{Rousseau:MIA:2010} incluant une approche de super-resolution visant à permettre à un plus grand nombre de détails d'être révélés.
...
Nous présentons une analyse quantitative des résultats ainsi qu'une étude succinte de l'apparition des différents sillons  cérébraux au cours de la grossesse.
...
\emph{Comparaison entre les segmentations d'images reconstruites selon (a) \cite{Rousseau:AR:2006} et (c) \cite{Rousseau:MIA:2010}. (b) et (d) sont les segmentations respectives de (a) et (c). Le principal effet notable est la correction de la sursegmentation du cortex.}
...
Le principal effet notable de l'utilisation de la super-résolution est la correction de la sursegmentation. 
...
Notre méthode de segmentation donnant de bons résultats concernant l'évaluation de la surface du cortex, la base de données diponible permet de réaliser une observation de l'apparition des sillons corticaux au cours du temps.
Cinq cas, acquis à respectivement 26, 27, 28, 30 et 32 semaines sont présentés afin d'illustrer cette évolution.

La figure~\ref{DER:3D} montre le cortex tel qu'il apparait à 26 semaines d'aménorrhée.
...
On peut observer cependant que la figure~\ref{AYD:3D} semble présenter un retard dans son développement, notamment vis-à-vis de la figure~\ref{TAB01:3D}, qui à une semaine d'aménorrhée de moins présente une gyration plus avancée.
Néanmoins, l'observation d'un tel écart dans le développement est tout à fait normal.
En effet, il est courant d'observer un écart de deux semaines entre l'apparation d'un sillon chez certains patients et sa présence dans 75\% des cas \cite{Glenn:SemPer:2009}.
De manière générale, l'observation de l'apparation des sillons est conforme aux observations médicales, ce qui laisse penser qu'il est possible de mettre en \oe uvre un suivi automatique de leur formation et de leur développement au cours de la grossesse.
