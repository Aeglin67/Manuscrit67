...
Le but de ce chapitre est de présenter une méthode de segmentation du cortex du cerveau à partir d'images IRM acquises au cours de la grossesse.
...
Plusieurs différences entre ces deux types d'images IRM expliquent ce constat.
...
Afin d'obtenir un rapport signal à bruit favorable, des voxels fortement anisotropiques sont définis (les caractéristiques des images utilisées sont décrites plus en détail en Section~\ref{subsubsec:foetus:param}), entrainant un important effet de volume partiel.
...
Dans un cas non mature, le cortex est une structure se plissant progressivement (lisse à 26 semaines de grossesses et très circonvolué à 32 semaines) où l'on peut observer la formation progressive des sillons.
Concernant la matière blanche, la myélinisation aparait autour de la vingt-cinquième semaine dans les pédoncules cérébraux inférieurs et se propage aux colliculi inférieurs, au cervelet postérieur et aux noyaux ventro-latéraux du thalamus \cite{Rutherford:2001}.
Par la suite, aucune progression de la myélinisation n'est observée au cours de la période allant de la vingt-huitième à la quarantième semaine, la matière blanche restant principalement composée d'eau, ce qui entraine un temps de relaxation $T2$ plus long et un signal plus élevé si l'image acquise est pondérée en T2.
...
Le plus notable est l'inversion des contrastes entre la matière blanche et la matière grise par rapport à une image adulte (le cortex apparaît plus foncé que la matière blanche dans  le cas adulte pondéré en T1 et également plus foncé dans le cas d'une image IRM f\oe tale pondérée en T2) (voir en Figure~\ref{FIG:DISTRIB}).
Dans le cadre d'une classification, l'effet de volume partiel (présence de plusieurs tissus dans un même voxel conduisant à une altération du signal) propre aux IRM conduit alors à des artefacts comme la présence d'une fine bande de voxels classés comme matière blanche entre le LCR et le cortex, ce qui n'est pas pertinent d'un point de vue anatomique (voir Figure~\ref{FIG:CLASSIF:FOETUS}).
...
\emph{Illustration de l'effet d'un hypersignal dans la matière blanche sur la segmentation. (a) Image IRM à niveau de gris. (b) Séparation du LCR interne et du LCR externe. Légende : LCR externe (rouge), cerveau (vert), LCR interne (bleu). (c) Segmentation du cortex. Légende : LCR (rouge), cortex (vert), matière blanche et noyaux gris (bleu).}
...
Cependant, une ambiguïté est introduite là où le quatrième ventricule rejoint le LCR externe, conduisant à sur-évaluer l'étendue de ce dernier et donc à définir une zone d'intérêt trop large, provoquant l'apparition de faux positifs dans la segmentation, surtout dans la région autour du cervelet et du tronc cérébral (voir Figure~\ref{FIG:CORTEX:UNBUILD:CORO}).
...
\caption[Segmentation de quatres coupes coronales consécutives d'une image IRM non-reconstruite]{
\emph{Segmentation de quatres coupes coronales consécutives d'une image IRM non-reconstruite. Il est possible d'observer sur ces coupes la connexion entre le LCR externe et le LCR interne et l'effet que produit cette connexion lors du processus de segmentation. Par exemple, le résultat de la segmentation (images (f) et (g)) montrent que le ventricule gauche a été classé comme LCR externe, entrainant la présence d'une mince couche de cortex autour de lui. Première ligne : images à niveau de gris. Deuxième ligne : segmentation. Légende : LCR (rouge), cortex (vert), matière blanche et noyaux gris (bleu).}
...
Cet exemple montre que la super-résolution permet à l'algorithme de segmentation de mieux détecter les limites entre la matière blanche et la cortex. 
...
Il pourrait par exemple être intéressant de préciser qu'il n'y a pas de cortex au dessus de tronc cérébral, ni autour des ventricules, permettant ainsi un relachement des contraintes géométriques autour de ces structures.
