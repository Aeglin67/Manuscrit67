...
Une analyse de différentes zones d'intérêt construites autour du cortex révèle des distributions en intensité des différents tissus plus favorable à une opération de partitionnement (par K-Moyennes ou par FCM), trois pics distincts apparaissant sur l'histogramme des intensités. 
...
Ce modèle est appelé le K-Moyennes topologique.

Rappelons tout d'abord que la segmentation par l'algorithme K-Moyennes est équivalente à la minimisation de la fonction d'énergie suivante : 
\begin{equation}
...
Tout comme l'algorithme FCM, le K-Moyennes ne prend pas en compte le biais en intensité présent dans les IRM.
...
% 	\subfigure[]{\includegraphics[height=45mm]{eps/chapitre4/inhomogeneity2.eps}}
	\includegraphics[height=45mm]{eps/chapitre4/process.eps}
        ...
        \caption[Schéma général du processus de segmentation]{\emph{Schéma du processus de segmentation. \`A partir de l'image originale, le K-Moyennes topologique permet de réaliser une segmentation séparant le LCR externe, le cerveau et les ventricules. \`A partir de la frontière entre le LCR externe et le cerveau, une zone d'intérêt incluant le cortex est définie. Le cortex est alors segmenté par un K-Moyennes topologique incluant l'intensité des voxels ainsi qu'une image issue du filtrage Top-Hat de l'image originale.}}

        \label{FIG:FOE:PROCESS}

\end{figure}

La méthode de segmentation du cortex se divise en deux phases distinctes, les deux utilisant le K-Moyennes topologique défini en section précédente (voir Figure~\ref{FIG:FOE:PROCESS}).
...
\begin{figure}[!t]
	...
	\subfigure[]{\includegraphics[height=33mm]{eps/chapitre4/Cortex_Seg_Final.eps}}
        ...
        \caption[Segmentation du cortex - Méthodologie]{\emph{Segmentation du cortex. (a) Schéma du processus. (b) Image originale. (c) En rouge, résultat du filtre Top-Hat dans la zone d'intérêt définie à partir de la frontière entre le LCR externe et le cerveau. (d) Zone d'intérêt et initialisation des labels LCR, cortex et matière blanche. (e) Segmentation finale obtenue par le K-Moyennes topologique. Légende (pour (d) et (e)) : LCR (bleu), cortex (jaune), matière blanche (rouge).}}
...
Cette initialisation est choisie car elle permet d'une part la construction d'une zone d'intérêt incluant le cortex, d'autre part de disposer d'une quantité de voxels de LCR et de matière blanche significative, de manière à pouvoir appliquer le K-Moyennes topologique en trois classes.
...
La segmentation est effectuée en exécutant l'algorithme K-Moyennes topologique avec deux entrées, qui sont l'image originale et l'image filtrée par le \emph{Top-Hat}.
...
Les expériences sont menées sur huit cas acquis avec un imageur IRM de 1.5 Tesla (Magnetom Avento, Siemens, Allemagne Erlangen) utilisant des séquences d'écho de spins ultra-rapides.
...
Ces images étant anisotropes, le K-Moyennes topologique évolue en deux dimensions et utilise un 4-voisinage pour l'évolution des différents labels. 
...
Un 6-voisinage 3D est utilisé pour mettre en \oe uvre l'algorithme de K-Moyennes topologique.
...
Cette section présente des résultats préliminaires issus de la segmentation de volumes reconstruits selon la méthode de \cite{Rousseau:MIA:2010} incluant une approche de super-résolution.
...
Une piste de réfléxion pourrait être de préciser qu'il n'y a pas de cortex au-dessus du tronc cérébral, ni autour des ventricules, permettant ainsi une adaptation des contraintes géométriques autour de ces structures.
